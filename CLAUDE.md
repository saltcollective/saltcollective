...
**All club resolution in `(app)` goes through `getClubAccess(locals, cookies)`** (`apps/hub/src/lib/server/club-access.ts`) — never write a raw `clubMembership.findFirst` for access resolution in `(app)` loads or actions (target-member lookups scoped by `clubId` are fine). The helper returns `{ club, role, impersonating }`: an open impersonation session wins for SITE_ADMINs, otherwise the user's own membership. Multi-club support will extend this same helper.
...
1. **Multi-club administration** — the schema already allows a user to hold multiple `ClubMembership`s, but the app assumes one. All `(app)` club resolution now flows through `getClubAccess()` (`apps/hub/src/lib/server/club-access.ts`) — multi-club means extending that one helper with a persisted "active club" selection (cookie) plus a club switcher in the hub shell. Onboarding also blocks users with an existing published club from creating another.
2. **Club billing lifecycle management** — track each club's billing state: last payment date/amount, next billing date, `paidUntil` (column already exists), plan, payment status. Clubs that fall out of a billing cycle get suspended — but today a suspended club's dashboard gives **no indication at all**: the club admin can keep editing while their public hub 404s. Two halves:
