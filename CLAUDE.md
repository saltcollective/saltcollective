...
2. **Admin club detail view** — `/admin/clubs` now has suspend/delete actions but no per-club detail screen (members, sponsors, activity at a glance). For the activity pane, deep-link `/admin/activity?entity=CLUB&id=...` rather than rebuilding it.
