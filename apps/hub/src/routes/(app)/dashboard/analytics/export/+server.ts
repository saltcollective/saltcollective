...
// Layout guards don't run for +server endpoints — this must resolve membership itself.
export const GET: RequestHandler = async ({ locals, url }) => {
  ...
  const where: { clubId: string; createdAt?: { gte: Date } } = { clubId: membership.club.id };
  ...
  return new Response(csv, {
    headers: {
      ...
      'Content-Disposition': `attachment; filename="${membership.club.slug}-clicks-${stamp}.csv"`,
    },
  });
};
